...
public class ResoursesGiftAnimation {
    ...
    public ResoursesGiftAnimation(MapScreen mapScreen, Group group, float x, float y, int resoursesCount) {
        this.mapScreen = mapScreen;
        this.x = x;
        this.y = y;
        this.resoursesCount = resoursesCount;
        this.group = group;
        gameManager = mapScreen.getGameManager();
        endXPos = -386;
//        endXPos = -320;
        if (resoursesCount == 2)
            endXPos = -640;
        endYPos = mapScreen.getHeight() / 3 - 128;
    }
